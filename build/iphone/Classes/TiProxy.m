...
//Rare exceptions to indicate a bug in the _navi_tour code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_navi_tour = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_navi_tour]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.navi_tour.module"
